...
//         const response1 = await request(app).post('/api/v1/signup').send(body1);
//         expect(createUserMock.mock.calls[0][0].email).toBe(body1.email);
//         expect(createUserMock.mock.calls[0][0].password).toBe(body1.password);
//       }
//     });
//     it('Parameter Check for isEmailNotInDBMock', async () => {
//       for (const body2 of bodyData) {
//         isEmailNotInDBMock.mockReset();
//         const response2 = await request(app).post('/api/v1/signup').send(body2);
